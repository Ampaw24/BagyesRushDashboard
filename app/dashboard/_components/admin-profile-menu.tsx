...
import { ChevronDownIcon, LogoutIcon } from "../_lib/icons";
import { adminRoleMeta } from "../_lib/status";
...
export function AdminProfileMenu({ id, name, role, showDetails = true }: AdminProfileMenuProps) {
      ...
      >
        ...
        {showDetails && (
          <span className="hidden flex-col items-start leading-tight sm:flex">
            ...
            <span className="text-xs text-text-muted">{adminRoleMeta[role].label}</span>
          </span>
        )}
      ...
      {open && (
        <div role="menu" className="absolute right-0 z-30 mt-1 flex w-56 flex-col gap-0.5 rounded-lg border border-border-subtle bg-surface p-1.5 shadow-sm">
          ...
          <Link
            href="/login"
            onClick={() => setOpen(false)}
            className="flex min-h-9 items-center gap-2 rounded-md px-3 text-sm text-status-critical transition duration-150 hover:bg-status-critical/10"
          >
